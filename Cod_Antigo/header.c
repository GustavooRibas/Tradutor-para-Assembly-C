...
short int pos[16]; // Posição dos elementos em uma pilha do vetor
...
void defLocalVar()
{
  ...
  while (1)
  {
    ...
    else if (!strcmp("reg", s1)) // Se for registrador
    {
      sscanf(str, "%s %2s %d", s1, s2, &ax);
      sscanf(str, "%s %s", s1, s2);
  ...
  /**/  if (cont % 8)
        ...
        {
          cont += 8;
          pos[7 + aux3] = cont; // Salva o endereço dos parâmetros
...
void salvaParam(){  //
  ...
  while (aux2)
  {
    printf("    movq %s, -%d(%s)\n", registradores64[5 - aux], pos[7 + aux], registradores64[6]);
    aux++;
    aux2--;
  }
}
...
void recupParam(){//
  ...
  while (aux2)
  {
    printf("    movq -%d(%s), %s\n", pos[7 + aux], registradores64[6], registradores64[5 - aux]);
    aux++;
    aux2--;
  }
}
...
void funcCall()
{
  ...
  sscanf(str, "%s %s %s %s %s %s %s", s1, s2, s3, s4, s5, s6, s7); //vi2 = call f1 pa1 pa2 pa3
  if (strlen(s5) > 1)
  {
