  ...
  while (1)
  {
    ...
    else if (!strcmp("enddef", s1)) // Salva os parametros na pilha
    {
      if (posparam != 0)
      {
        ...
        {
          cont += 8;
          pos[18 + aux3] = cont; // Salva o endereço dos parâmetros
...
void funcExpressoes()
{
  ...
  if (strlen(s5) > 2) // Expressões complexas
  {
    ...
    else if (!strcmp("vr", s7)){ //Caso a variável que recebe seja um registrador inteiro   (alterado)
  ...
  else // Expressões simples
  {
    ...
    {
      if (!strcmp("vr", s2)){ //Caso quem receberá a atribuição seja um registrador inteiro (alterado)
    ...
    else if (!strcmp("vr", s7)){ //Caso o valor atribuido seja um registrador inteiro (alterado)
