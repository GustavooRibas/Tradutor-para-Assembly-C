  ...
  if (strlen(s6) > 1)
  {
    ...
    else if (s2[1] == 'i') // Para vi (variável inteira) ou pi (parâmetro inteiro)
    {
      if (!strcmp(s2, "vi")) // Variável inteira
        ...
        printf("    movl -%d(%s), %s\n", pos[17 + ax], registradores64[6], registradores32[4]); // Parâmetro inteiro
      }
    }
    else if (s2[1] == 'r'){ // Para variáveis de registrador

      if(!strcmp(s2, "vr")){ // Variável inteira de registrador
          printf("    movl %s, %s\n", vet_aux_reg_verde[ax - 1], registradores32[4]); // Registrador inteiro
      }
    }
  }
  ...
  if (strlen(s7) > 1)
  {
    ...
    else if (s2[1] == 'i') // Para vi (variável inteira) ou pi (parâmetro inteiro)
    {
      if (!strcmp(s2, "vi")) // Variável inteira
        ...
        printf("    movl -%d(%s), %s\n", pos[17 + ax], registradores64[6], registradores32[3]); // Parâmetro inteiro
      }
    }
    else if (s2[1] == 'r'){ // Para variáveis de registrador

      if(!strcmp(s2, "vr")){ // Variável inteira de registrador
          printf("    movl %s, %s\n", vet_aux_reg_verde[ax - 1], registradores32[3]); // Registrador inteiro
      }
    }
  }
...
void cmplCondicionalIf(){
  ...
  if (!strcmp("vi", s1)){ // Se o termo 'A' for variável inteira em "A <comparação> B"
      ...
      if (!strcmp("vi", s5)){ // Se o termo 'B' for variável inteira em "A <comparação> B"
        printf("    movl -%d(%s), %s\n\n", pos[ax - 1], registradores64[6], registradores32[8]);
        printf("    cmpl -%d(%s), %s\n", pos[a1 - 1], registradores64[6], registradores32[8]);
      }
